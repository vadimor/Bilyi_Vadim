...
Ccooperator* Clist::operator[](int i)
{
	if (size > i)
		return mass[i];
}

ostream& operator<<(ostream& output, Clist& obj)
{
	int n = obj.getSize();
	for (int i = 0; i < n; i++)
		output << obj.getEl(i)->getString();
	return output;
}

ofstream& operator<<(ofstream& output, Clist& obj)
{
	int n = obj.getSize();
	for (int i = 0; i < n; i++)
		output << obj.getEl(i)->getString();
...
istream& operator>>(istream& input, Clist& obj)
{
	Ccooperator* temp;
		temp = new CcoopK;
	std::stringstream ss1;
	...
	int tid1;
	string tname1 = " ";
	input >> tid >> tage >> tsalary >> tweight >> tname >> trectage >> trectname >> pork;
	ss1 << tid;
	ss1 >> tid1;
	temp->setId(tid1);
	ss1.clear();
...
ifstream& operator>>(ifstream& input, Clist& obj)
{
	std::regex regular_main(
		"([0-9]+[ \\f\\n\\r\\t\\v]{1})"//id
		"([0-9]+[ \\f\\n\\r\\t\\v]{1})"//age
