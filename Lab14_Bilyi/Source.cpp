...
using std::make_pair;
using std::find_if;
void creat_second_vector(vector<C_Rgz*>& vect);
void creat_second_map(map <int, C_Rgz*>& mp);
void menu();
class Functor {
public:
	bool operator()(C_Rgz* a, C_Rgz* b) {
		return *a > *b;
	}
};
C_Rgz* tempclass;
...
void menu() {
	...
	vector<C_Rgz*>sec_vect;
	char ch;
	int count=0;
	while (choose != -1) {
		cout << "\n1-add\n2-delete\n3-show element\n4-show all\n5-find el\n6-count elem\n7-creat second vaector\n8-sort\n\n-1-exit\nchoose way: ";
		cin >> choose;
		switch (choose)
		{
		case 1:
		...
		case 6:
			...
			delete tempclass;
		case 7: 
			creat_second_vector(sec_vect);
			cout <<endl<< "combine vectors (y/n): ";
			cin >> ch;
			if (ch == 'y') {
				vect.insert(vect.end(), sec_vect.begin(), sec_vect.end());
	...
	while (choose != -1) {
		...
		case 6:
			...
			break;
		case 7:
			creat_second_map(sec_map);
			cout << "\ncombine vectors (y/n): ";
			cin >> ch;
			if (ch == 'y') {
				for (auto var : sec_map) {
					for (int i = 0; i < keymass.size(); i++) {
						if (keymass[i] == var.first) {
							*mp.find(keymass[i])->second += *var.second;
							prz1=1;
							delete var.second;
						}
					}
